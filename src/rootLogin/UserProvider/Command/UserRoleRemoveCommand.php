...
class UserRoleRemoveCommand extends Command
{
    ...
    protected function configure()
    {
        $this
            ...
            ->setHelp(<<<EOT
The <info>user:role:remove</info> removes a role from an user:
  <info>php app/console user:role:remove test@example.org</info>
This interactive shell will ask you for the role you want to remove.
You can alternatively specify the role as the second:
  <info>php app/console user:role:remove test@example.org ROLE_ADMIN</info>
EOT
            );
    }
    /**
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if(!$user->hasRole($role)) {
            $output->writeln(sprintf('User <comment>%s</comment> hasn\'t the requested role <comment>%s</comment>.', $email, $role));
            return 0;
        }
        ...
        $output->writeln(sprintf('Removed role <comment>%s</comment> from user <comment>%s</comment>', $role, $email));
    }
    /**
     * @see Command
     */
